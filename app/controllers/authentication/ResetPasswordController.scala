...
class ResetPasswordController @Inject() (
  ...
  def view(token: UUID) = silhouette.UnsecuredAction.async { implicit request =>
    authTokenService.validate(token).map {
      case Some(authToken) => Ok(views.html.authentication.resetPassword(ResetPasswordForm.form, token))
    ...
    authTokenService.validate(token).flatMap {
      case Some(authToken) =>
        ResetPasswordForm.form.bindFromRequest.fold(
          form => Future.successful(BadRequest(views.html.authentication.resetPassword(form, token))),
